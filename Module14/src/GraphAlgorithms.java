...
public class GraphAlgorithms {
    ...
    public static <T> Set<Edge<T>> prims(Vertex<T> start, Graph<T> graph) {
        ...
        for (VertexDistance<T> w : graph.getAdjList().get(start)){
            queue.add(new Edge<>(start, w.getVertex(), w.getDistance()));
        }
        visitedSet.add(start);
        while ((!queue.isEmpty()) && (visitedSet.size()<graph.getVertices().size())){
            Edge<T> edges = queue.remove();
            if (!visitedSet.contains(edges.getV())){
                visitedSet.add(edges.getV());
                MST.add(edges);
                MST.add(new Edge<>(edges.getV(), edges.getU(), edges.getWeight()));
                for (VertexDistance<T> w: graph.getAdjList().get(edges.getV())){//each adjacent vertex
                    if (!visitedSet.contains(w.getVertex())){
                        queue.add(new Edge<>(edges.getV(), w.getVertex(), w.getDistance()));
                    }
                }
            }
